...
private:
	...
	{
		this->scene = scene;
		this->directory = ""; // O asigna si necesitas cargar texturas

		// Importante: transforma raíz inversa si lo necesitas
		aiMatrix4x4 inverseTransform = node->mTransformation;
		inverseTransform.Inverse();
		m_GlobalInverseTransform = aiMatrix4x4ToGlm(inverseTransform);

		// Procesa el nodo y sus hijos (como en loadModel original)
